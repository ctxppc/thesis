...
\begin{listing}[ht]
	...
	\caption{Assembly code of the seal creation routine as inserted in each program. The CHERI LLVM assembler did not support the \iasm/cclear/ instruction at the time of development so it's encoded directly by Glyco instead. The \g{sealcap} is stored at \texttt{mm.cseal_seal_cap} and is initialised by the initialisation routine (which is part of the \g{rt}).}
	\label{lst:cseal}
\end{listing}
