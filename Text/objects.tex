...
Object and method capabilities are sealed using the same \g{sealcap}; a \g{methcap} can therefore be invoked on an \g{objcap} using the \textbf{CInvoke} CHERI-RISC-V instruction. This immediately poses an issue in protecting the \g{sealcap} while at the same still allowing arbitrary clients to create new objects and thus derive new \gs{objcap}.
