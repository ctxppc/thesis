...
\todo{Add assembly excerpt of example (if informative).}
...
\paragraph{From CD to PR} The CD to PR (Predicates) \g{nanopass} flattens conditionals, sequences, and procedures into \gs{bblock}. A \textbf{\g{bblock}} is \glsdesc*{bblock}. A \g{bblock}'s effects are followed by a \textbf{continuation}, which is a jump, branch, or call. For example, the CD program
...
\paragraph{From MM to RV} MM introduces the call stack and heap and operations on them such as call frame pushing and popping and buffer allocation.
...
\paragraph{From RV to S} An RV program consists of CHERI-RISC-V instructions. The RV to S (CHERI-RISC-V Assembly) \g{nanopass} serialises these instructions. For example, the \iil/copyWord(destination: a0, source: a1)/ instruction is serialised as \hbox{\texttt{mv a0, a1}}.
...
\todo{Add example, including \g{lowering} to assembly.}
