...
Glyco is built following a \emph{nanopass} compiler design, described in the context of compiler education by several authors such as \cite{educomp} and in the context of commercial compiler development by authors such as \cite{commcomp}. \shb{prefer to use \citep here and drop the "by several authors such as"} A nanopass compiler consists of numerous small passes, so-called \textbf{\glspl{nanopass}}, which translate programs written in one \textbf{\g{il}} to programs written in another. The input of a \g{nanopass} is a program in a higher-level language than the one of its output; we therefore call this translation process a \textbf{\g{lowering}}. A \g{nanopass} should be concise and perform a relatively self-contained transformation. There should be no data structures shared across \gs{nanopass} except for the program itself.
