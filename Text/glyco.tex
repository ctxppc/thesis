...
\paragraph{From CC (Calling Convention) to SV} CC introduces procedures with parameters and results. It is \lowered{} to its \g{lowerlang} SV by imposing the \g{cc} described above. We'll work through the CC to SV \g{nanopass} by applying it to the following CC program.
...
The \texttt{call} effect is \lowered{} by copying the arguments to the appropriate registers (or newly allocated locations on the call stack), passing control to the callee, and finally (when control is returned) copying the result from \texttt{a0} to the result location. The register allocator (discussed in \cref{sct:id}) needs to know what registers are used for passing arguments and are thus live across the call; the \texttt{call} effect is therefore annotated with a list of used arguments registers.
...
The procedure begins analogously to the program by pushing a new scope and binding the caller's \g{retcap} to an abstract location. It does the same for all \gs{ceesaved}, since their contents must be preserved by a procedure before it can use them.
...
\paragraph{From RV (CHERI-RISC-V) to S (CHERI-RISC-V Assembly)} An RV program consists of CHERI-RISC-V instructions. The RV to S \g{nanopass} serialises these instructions. For example, the \iil/copyWord(destination: a0, source: a1)/ instruction is serialised as \hbox{\texttt{mv a0, a1}}.
