...
A procedure call in \g{ghscc} is realised using a \textbf{secure calling (s-call) routine} and is therefore referred to as an \textbf{\g{scall}}. This \g{rtrt} expects a target capability in \texttt{ct6}, a \g{retcap} in \texttt{cra}, and a \g{framecap} in \texttt{cfp}. When invoked, the routine generates a unique \g{sealcap}, seals the frame and return capabilities, and jumps to the target capability's address. Similar to how the heap capability in the allocation routine grants the latter access to the heap and points to the next free location, the scall routine contains a \g{sealcap} that grants it \g{sealing} power for all \gs{otype} and whose address is the next available \g{otype}.

Just like in \g{gccc}, the callee receives the caller's \g{framecap} and a \g{retcap} to the caller. Unlike \g{gccc} however, both capabilities are sealed with an \g{otype} for which the callee has no \g{unsealcap}. The callee returns to the caller using the \texttt{CInvoke} instruction, passing to it the sealed return–frame capability pair. The \texttt{CInvoke} instruction checks whether both capabilities have the same \g{otype} (which they have iff they're provided by the scall routine), unseals them, puts the unsealed data capability (the \g{framecap}) in \texttt{ct6}, and jumps to the address of the code capability (the \g{retcap}). The caller moves the capability in \texttt{ct6} back to \texttt{cfp} and resumes execution. This has three consequences.

First and foremost, the callee cannot dereference the \g{framecap} and thus cannot access the caller's local state, thereby preserving \g{lse}. The capability is only unsealed when control is returned to the caller.

Second, the callee cannot dereference the \g{retcap} or use it to jump to its address using the normal jump instruction \texttt{CJALR} or variants. It can only return to the caller by also providing a \g{framecap} sealed with the same \g{otype}, i.e., sealed by the same \g{scall}. It cannot forge a call frame and pass the caller a capability to it.
