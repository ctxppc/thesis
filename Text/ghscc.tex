...
A \g{cc} proposed by \cite[section~7.3]{cerise} provides \g{lse} by ensuring that call frames always occupy freshly allocated memory to which certainly no dangling capabilities point. However it does not provide \g{wbcf}. \g{ghscc} adapts a variant of this \g{cc} that provides \g{lse} and a weaker variant of \g{wbcf}.
...
In this section we evaluate the implementation of \g{ghscc} in terms of (relative) language and codebase growth which can serve as an indicator to the development effort required for adding a new \g{cc} to an existing \g{nanopass} compiler with a similar design. We then evaluate \g{ghscc} itself by benchmarking a few test programs against the previous \g{cc}.
...
\begin{table}[h]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Benchmarking test programs in \g{gccc} and \g{ghscc} in Glyco 0.2. The program size (in number of assembly statements) is split between the runtime and the rest of the program.}
...
The heap-based \g{cc} proposed in this chapter and implemented in Glyco is but one \g{cc} providing \g{lse}, albeit without a rigorous proof. Several alternatives have been proposed in literature or are implemented in CHERI software.
