...
We now evaluate the impact of the implementation of \g{ghscc} in Glyco which can serve as an indicator to the development effort required for adding a new \g{cc} to an existing \g{nanopass} compiler with a similar design. We first compare the codebase's growth present the most significant changes to Glyco's languages, going from low-level to high-level \gs{il}, then quantify the changes.
