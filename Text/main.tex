...
\setup{
	masteroption=vs,
	title={Glyco: A CHERI-RISC-V nanopass compiler for iterating quickly on capability-based security features},
...
\begin{abstract}
	This thesis explores a design \& implementation of a \g{nanopass} compiler producing CHERI-RISC-V executables that use a secure \g{cc} guaranteeing \g{lse} and a variation of \g{wbcf} called \emph{unrepeatable return}. The secure \g{cc} is then compared in terms of runtime and memory overhead against a more traditional \g{cc} using a few test programs. The thesis also proposes and implements a design for machine-enforced object encapsulation in a feature we call \emph{sealed objects}. Finally, the \g{nanopass} approach itself is evaluated by assessing the impact that each new security feature has on the compiler's codebase and pipeline.
\end{abstract}
