(
	...
	functions: (
		...
		in: let((ob.seal, field(seal, of: ob.self)), in: value(sealed(record((one, one) (term, term)), with: ob.seal)))
	)
	(
		l.anon$1,
		takes: (ob.self, cap(record(((one, s32) (term, s32)), sealed: true)), sealed: true) (term, s32, sealed: false),
		returns: s32,
		in: let((one, field(one, of: ob.self)) (term, field(term, of: ob.self)), in: value(binary(one, add, term)))
	)
)