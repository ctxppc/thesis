(
	do(
		...
		set(ls.rec$1, to: ls.typeobj)
		set(ls.val$1, to: ls.seal)
		setField(seal, of: ls.rec$1, to: ls.val$1)
	...
	procedures: (
		...
		in: do(
			...
			set(ls.rec$2, to: ls.self$1)
			set(ls.val$1, to: ls.initialValue)
			setField(value, of: ls.rec$2, to: ls.val$1)
			set(ls.cap, to: ls.self$1)
			set(ls.seal$1, to: ls.seal)
