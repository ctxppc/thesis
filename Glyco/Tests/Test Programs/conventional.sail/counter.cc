(
	do(
		...
		set(ls.rec$1, to: location(ls.typeobj))
		set(ls.val$1, to: location(ls.seal))
		setField(seal, of: ls.rec$1, to: location(ls.val$1))
		...
		call(location(ls.f$1), location(ls.arg$2), result: ls.ignored)
		set(ls.arg$3, to: location(ls.counter))
		set(ls.f$2, to: location(ls.Counter.increase.m))
		call(location(ls.f$2), location(ls.arg$3), result: ls.ignored$1)
	...
	procedures: (
		...
		in: do(
			...
			createRecord(((value, s32())), capability: ls.self$1, scoped: true)
			set(ls.rec$2, to: location(ls.self$1))
			set(ls.val$1, to: location(ls.initialValue))
			setField(value, of: ls.rec$2, to: location(ls.val$1))
			set(ls.cap, to: location(ls.self$1))
			set(ls.seal$1, to: location(ls.seal))
