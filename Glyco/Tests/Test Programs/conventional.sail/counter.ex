(
	value(
		let(
			(ob.seal, seal())
			(ob.Counter.init, function(l.anon))
			...
			in: let(
				(counter, evaluate(named(ob.Counter.init), constant(32)))
		...
		in: value(
			sealed(
				let(
					...
					in: do(setField(value, of: named(self), to: named(initialValue)), then: named(self))
				),
				with: named(ob.seal)
			)
		)
