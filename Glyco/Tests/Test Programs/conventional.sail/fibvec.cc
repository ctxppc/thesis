	...
	(
		...
		in: if(
			...
			then: do(
				...
				set(ls.idx$2, to: location(ls.index))
				set(ls.vec$3, to: location(ls.nums))
				set(ls.lhs$1, to: location(ls.index))
				set(ls.rhs$1, to: constant(2))
				compute(ls.idx$3, location(ls.lhs$1), sub, location(ls.rhs$1))
				getElement(of: ls.vec$3, index: location(ls.idx$3), to: ls.lhs$2)
				set(ls.vec$4, to: location(ls.nums))
				set(ls.lhs$3, to: location(ls.index))
				set(ls.rhs$2, to: constant(1))
				compute(ls.idx$4, location(ls.lhs$3), sub, location(ls.rhs$2))
				getElement(of: ls.vec$4, index: location(ls.idx$4), to: ls.rhs$3)
				compute(ls.elem$1, location(ls.lhs$2), add, location(ls.rhs$3))
				setElement(of: ls.vec$2, index: location(ls.idx$2), to: location(ls.elem$1))
