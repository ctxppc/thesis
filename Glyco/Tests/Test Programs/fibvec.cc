(
	...
	procedures: (
		...
		in: do(
			...
			pushVector(s32(), count: 30, capability: ls.arg2)
	...
	(
		...
		in: if(
			...
			then: do(
				...
				compute(ls.indexOfFirst, location(ls.lhs$1), sub, location(ls.rhs$1))
				set(ls.lhs$2, to: location(ls.index))
				set(ls.rhs$2, to: constant(1))
				compute(ls.indexOfSecond, location(ls.lhs$2), sub, location(ls.rhs$2))
				set(ls.lhs$3, to: location(ls.index))
				set(ls.rhs$3, to: constant(1))
				compute(ls.nextIndex, location(ls.lhs$3), add, location(ls.rhs$3))
				...
				compute(ls.fibNum, location(ls.lhs$4), add, location(ls.rhs$4))
