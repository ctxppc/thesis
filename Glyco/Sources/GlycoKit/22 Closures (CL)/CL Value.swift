	...
	public enum Value : PartiallyStringCodable, PartiallyIntCodable, SimplyLowerable, Element {
		...
		func lowered(in context: inout Context) throws -> Lower.Value {
			switch self {
				...
				let (loweredBody, capturedNamesAndTypes) = try context.withClosure(parameters: parameters, closure: body.lowered)
				if capturedNamesAndTypes.isEmpty {
					...
					return .letType([
						try .object(.init(
							...
							constructor:	.init(
								...
								in:		.record(capturedNamesAndTypes.map { (name, _) in
							...
							methods:		[
								.init(
									...
									in:			.let(
										capturedNamesAndTypes.map { (name, _) in
		...
		func type(in context: Context) throws -> ValueType {
			switch self {
				...
				var context = context
				let (_, capturedNamesAndTypes) = try context.withClosure(parameters: parameters, closure: result.lowered)
				return .cap(.function(takes: parameters, returns: resultType, closure: !capturedNamesAndTypes.isEmpty))
				...
				switch try target.type(in: context) {
					...
					case .cap(.function(takes: _, returns: let resultType, closure: _)),
