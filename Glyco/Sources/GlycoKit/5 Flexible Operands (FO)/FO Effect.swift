	...
	public enum Effect : Codable, Equatable, MultiplyLowerable {
		...
		public func lowered(in context: inout ()) throws -> [Lower.Effect] {
			switch self {
				...
				let (loadSource, source) = try load(.cap, from: .init(source), using: tempRegisterA)
				let (loadSeal, seal) = try load(.cap, from: .init(seal), using: tempRegisterB)
				let (storeResult, destination) = try store(.cap, to: destination, using: tempRegisterA)
				loadSource
				loadSeal
				Lower.Effect.seal(into: destination, source: source, seal: seal)
				storeResult
